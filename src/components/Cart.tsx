...
const Cart = () => {
  ...
  const handleDec = (id:number) => {
    ...
    const updatedCart = carts.map((item) => {
      // if(item.quantity === 0) removeProduct(item.id)
      if (item.id === id) {
        return {
          ...item,
          quantity: item.quantity - 1,
        };
        // can you implement the logic when cart item quantity is 0 call removeProduct method?
      }
      return item;
    });
    localStorage.setItem("cart", JSON.stringify(updatedCart));
  ...
  const deliveryChargeHandler = (e:any) => {
    // console.log("delivery", e.target.value);
  ...
  return (
    <div className="container mx-auto pt-20">
      <div className="flex shadow-md my-10">
        ...
        <div id="summary" className="w-1/4 px-8 py-10 text-white">
          ...
          <div>
            ...
            <select defaultValue={deliveryCharge} onChange={(e) => deliveryChargeHandler(e)} className="block p-2 text-gray-600 w-full text-sm">
              <option value={2}>Normal shipping - $2.00</option>
          ...
          {/* <button className="bg-red-500 hover:bg-red-600 px-5 py-2 text-sm text-white uppercase mt-6">
            Apply
          </button> */}
          <div className="border-t mt-8">
            <div className="flex font-semibold justify-between py-6 text-sm uppercase">
              <span>Total cost </span>
              <span>$ {(total + parseInt(deliveryCharge)).toFixed(2)}</span>
            </div>
